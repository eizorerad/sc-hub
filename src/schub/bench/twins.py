...
def twin(source: str | os.PathLike, stratify: str | None = None, keep: Sequence[str] = (), fraction: float = 0.05,
    ...
    try:
        rows = select(labels, adata.n_obs, fraction, min_per_group, [str(k) for k in keep], max_keep, max_cells,
                      np.random.default_rng(seed))
        subset = adata[rows].to_memory()
    finally:
        adata.file.close()
    folder.mkdir(parents=True, exist_ok=True)
    partial = folder / ".data.h5ad.partial"
    subset.write_h5ad(partial)
    ...
    write_json_atomic(folder / "twin.json", {**rule, "n_obs_source": int(len(labels) if labels is not None else
                                                                          subset.n_obs / max(fraction, 1e-9)),
                                             "n_obs_twin": int(subset.n_obs), "groups": groups, "created": stamp()})
    print(f"twin of {path.name}: {subset.n_obs} cells -> {folder / 'data.h5ad'}")
    return folder / "data.h5ad"
