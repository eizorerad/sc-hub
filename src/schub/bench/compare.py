...
tolerance (10% unless given, per metric if a dict), or against the interval the
registration named for that metric (`bounds={"ratio": (0.32, 0.62)}`). The table is printed, so a
...
def _row(metric: str, paper: float, ours: float | None, tolerance: float,
    ...
    if bounds is not None:
        low, high = bounds
        verdict = f"{'within' if low <= ours <= high else 'outside'} [{low:.4g}, {high:.4g}]"
    else:
...
def _bounds(metric: str, value: Any) -> tuple[float, float]:
    try:
        low, high = (_number(f"bounds[{metric!r}]", v) for v in value)
    except (TypeError, ValueError) as exc:
        raise CompareError(f"bounds[{metric!r}] must be (low, high)") from exc
    if low > high:
        raise CompareError(f"bounds[{metric!r}]: low {low} is above high {high}")
    return low, high


def compare(ours: Mapping[str, Any], paper: Mapping[str, Any], source: str,
            tolerance: float | Mapping[str, float] = 0.10, name: str = "comparison",
            bounds: Mapping[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    intervals = {str(k): _bounds(str(k), v) for k, v in (bounds or {}).items()}
    rows = []
