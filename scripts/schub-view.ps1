...
function Update-View {
    # Rebuild on the cluster and list its images, cell maps and notebooks: the page is
    # fetched every time, the rest (most of the bytes) only when that list changes.
    $list = 'schub/bin/schub dashboard >/dev/null && cd schub/view && find img pts nb -type f -printf ''%p %s %T@\n'' 2>/dev/null | sort | cksum'
