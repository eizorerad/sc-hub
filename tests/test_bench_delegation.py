...
def test_a_chat_hands_a_task_over_and_follows_it(lab: Settings, cluster: FakeCluster) -> None:
    ...
    assert "Handed over by the student's assistant" in config.objective
    assert cluster.names[answer.job].startswith("schub-goal-p")
    state = delegation(lab, Slurm(cluster), "p")
    assert state.state == "queued" or state.state == "working"
    stopped = delegation(lab, Slurm(cluster), "p", stop=True)
...
def test_goal_files_know_the_mode() -> None:
    assert parse_goal("an objective long enough").mode == "free"
...
def test_a_free_claude_has_its_tools_in_a_sandbox_that_hides_keys() -> None:
    ...
    assert {"Bash", "Edit", "Task", "mcp__schub__*"} <= set(allowed)
    settings = json.loads(argv[argv.index("--settings") + 1])
    sandbox = settings["sandbox"]
    assert sandbox["enabled"] and not sandbox["allowUnsandboxedCommands"] and sandbox["network"]["allowedDomains"] == ["*"]
    assert "~/.ssh" in sandbox["filesystem"]["denyRead"] and "Read(~/.ssh/**)" in settings["permissions"]["deny"]
...
def test_a_free_codex_writes_only_in_its_folder() -> None:
    ...
    assert 'sandbox_mode="workspace-write"' in argv and "sandbox_workspace_write.network_access=true" in argv
...
def test_a_free_turn_works_in_the_project_and_its_own_work_is_journaled(lab: Settings, cluster: FakeCluster,
    ...
    [call] = engine_calls()
    assert result == "ok" and Path(call["cwd"]).resolve() == (lab.projects_dir / "p").resolve()
    ...
    assert [(f.path, f.change) for f in cell.files] == [("notes/plan.md", "created")]
...
def test_a_turn_that_only_used_the_sc_hub_tools_adds_no_cell(settings: Settings, tmp_path: Path) -> None:
    ...
    assert cell.status == "error" and "boom" in cell.message and cell.outputs[0].truncated > 3000
